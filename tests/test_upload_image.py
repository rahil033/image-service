...
class TestUploadImage(BaseTestCase):
    ...
    def test_upload_image_success(self, mock_service):
        ...
        event = self.create_api_event(method='POST', body={
            'user_id': 'user123',
    ...
    def test_upload_image_missing_required_fields(self):
        ...
        event = self.create_api_event(method='POST', body={'user_id': 'user123'})
        response = lambda_handler(event, self.mock_context)
        self.assertError(response, 400)
    
    def test_upload_image_invalid_json(self):
        """Test upload with invalid JSON."""
        event = {'httpMethod': 'POST', 'body': 'invalid json{', 'headers': {'Content-Type': 'application/json'}}
        response = lambda_handler(event, self.mock_context)
        self.assertError(response, 400)
    ...
    @patch('src.handlers.image_handler.service')
    def test_upload_image_storage_error(self, mock_service):
